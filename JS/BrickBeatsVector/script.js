...
var MAX_N_AMMUNITION = 50;
var MAX_N_ENEMIES    = 10;
...
function projectile_path(angle, mode) {
   var slope = Math.tan(angle * Math.PI / 180);

    var cells_on_path = [];

    if (mode == 999) {
        current_r = CELL_I_POS.Y;
        current_c = CELL_I_POS.X;
    } else {
        current_r = ENEMY_INFO[mode][0];
        current_c = ENEMY_INFO[mode][1];
    }

    if (angle == 0) {
        for (var i = (current_c + 1); i <= CELL_COLS_N; i++) {
            cells_on_path.push([current_r, i]);
        }
    } else if (angle == 90) {
        for (var i = (current_r + 1); i <= CELL_ROWS_N; i++) {
            cells_on_path.push([i, current_c]);
        }
    } else if (angle == 180) {
        for (var i = (current_c - 1); i > 0; i--) {
            cells_on_path.push([current_r, i]);
        }
    } else if (angle == 270) {
        for (var i = (current_r - 1); i > 0; i--) {
            cells_on_path.push([i, current_c]);
        }
    } else {
        var ref_offset_main;
        ...
        if (angle > 90 && angle < 270) {
            ref_offset_main = [0.40, 0.00, -0.40];
            ref_offset_aux  = [0.45, 0.05, -0.35];
            ref_angle       = [180,   270,    90];
            angle_mode      = 1;
            i = current_c - 1;
        ...
        while ((angle_mode == 1 && i > 0) || (angle_mode == 2 && i <= CELL_COLS_N)) {

            var j1 = ((i + ref_offset_main[0] - current_c) * slope) + current_r;
            var j2 = ((i + ref_offset_main[1] - current_c) * slope) + current_r;
            var j3 = ((i + ref_offset_main[2] - current_c) * slope) + current_r;
            if ((j1 % 0.5) == 0) {
                j1 = ((i + ref_offset_aux[0] - current_c) * slope) + current_r;
            }
            if ((j2 % 0.5) == 0) {
                j2 = ((i + ref_offset_aux[1] - current_c) * slope) + current_r;
            }
            if ((j3 % 0.5) == 0) {
                j3 = ((i + ref_offset_aux[2] - current_c) * slope) + current_r;
            }
            j1 = Math.round(j1);
            j2 = Math.round(j2);
            j3 = Math.round(j3);
            if (j1 > 0 && j1 <= CELL_ROWS_N) {
                if (cells_on_path.length == 0) {
                    cells_on_path.push([j1, i]);
                } else if (cells_on_path[cells_on_path.length - 1][0] != j1 
                       ||  cells_on_path[cells_on_path.length - 1][1] != i) {
                    cells_on_path.push([j1, i]);
                }
            }
            if (j2 > 0 && j2 <= CELL_ROWS_N) {
                if (cells_on_path.length == 0) {
                    cells_on_path.push([j2, i]);
                } else if (cells_on_path[cells_on_path.length - 1][0] != j2 
                       ||  cells_on_path[cells_on_path.length - 1][1] != i) {
                    cells_on_path.push([j2, i]);
                }
            }
            if (j3 > 0 && j3 <= CELL_ROWS_N) {
                if (cells_on_path.length == 0) {
                    cells_on_path.push([j3, i]);
                } else if (cells_on_path[cells_on_path.length - 1][0] != j3 
                       ||  cells_on_path[cells_on_path.length - 1][1] != i) {
                    cells_on_path.push([j3, i]);
                }
            }
            if (cells_on_path.length == 0) {
                if (Math.abs(slope) < 1) {
                    return projectile_path(ref_angle[0], mode);
                } else {
                    if (slope > 0) {
                        return projectile_path(ref_angle[1], mode);
                    } else {
                        return projectile_path(ref_angle[2], mode);
                    }
                }
            }
    ...
    for (var i = 0; i < cells_on_path.length; i++) {
        ...
        if (CELL_I_IS_BRICK[cells_on_path[i][0]][cells_on_path[i][1]]) {
            return [1, cells_on_path[i][0], cells_on_path[i][1]];
        } else if (mode == 999 && CELL_I_IS_ENEMY[cells_on_path[i][0]][cells_on_path[i][1]]) {
            return [2, cells_on_path[i][0], cells_on_path[i][1]];
        } else if (CELL_I_POS.Y == cells_on_path[i][0] && CELL_I_POS.X == cells_on_path[i][1]) {
            return [2, cells_on_path[i][0], cells_on_path[i][1]];
        }
    }
...
function shoot_enemy_vector(angle) {
    ...
    var projectile_path_return_value = projectile_path(angle, 999);

    shot_cell = [projectile_path_return_value[1], 
                 projectile_path_return_value[2]];

    if (projectile_path_return_value[0] == 1) {

        CELL_I_IS_BRICK[shot_cell[0]][shot_cell[1]] = false;
        print_message(MSG_BRICK_DOWN +  'R-' + shot_cell[0].toString() 
                                     + ' C-' + shot_cell[1].toString());

        var brick_cell_i = document.getElementById(get_id_str(shot_cell[0], shot_cell[1]));

        brick_cell_i.style.backgroundColor = '#' + COLOR_ENEMY_DEFEAT;

        setTimeout(function() {
            clear_cell(shot_cell[0], shot_cell[1]);
        }, ONE_SECOND);

    } else if (projectile_path_return_value[0] == 2) {

        N_ENEMIES--;

        CELL_I_IS_ENEMY[shot_cell[0]][shot_cell[1]] = false;
        print_message(MSG_ENEMY_DOWN +  'R-' + shot_cell[0].toString() 
                                     + ' C-' + shot_cell[1].toString());

        var enemy_cell_i = document.getElementById(get_id_str(shot_cell[0], shot_cell[1]));
        ...
        for (var j = 0; j < ENEMY_INFO.length; j++) {
            if (ENEMY_INFO[j][0] == shot_cell[0] 
            &&  ENEMY_INFO[j][1] == shot_cell[1]) {
    ...
    ENEMY_TRACK_TIMER = setInterval(function() {
        for (var i = 0; i < ENEMY_INFO.length; i++) {
            ...
            if (ENEMY_INFO[i][4] != 0) {
                ...
                var projectile_path_return_value = projectile_path(ENEMY_INFO[i][2], i);

                captured_cell = [projectile_path_return_value[1], 
                                 projectile_path_return_value[2]];

                if (projectile_path_return_value[0] == 1) {

                    CELL_I_IS_BRICK[captured_cell[0]][captured_cell[1]] = false;
                    print_message(MSG_BRICK_DOWN +  'R-' + captured_cell[0].toString() 
                                                 + ' C-' + captured_cell[1].toString());

                    var brick_cell_i = document.getElementById(get_id_str(captured_cell[0], captured_cell[1]));

                    brick_cell_i.style.backgroundColor = '#' + COLOR_ENEMY_DEFEAT;

                    setTimeout(function() {
                        clear_cell(captured_cell[0], captured_cell[1]);
                    }, ONE_SECOND);
                ...
                if (projectile_path_return_value[0] != 2) {
                    N_ENEMIES--;
...
function load_game() {
    MAX_N_AMMUNITION = parseInt(CELL_ROWS_N * CELL_COLS_N / 2);
    MAX_N_ENEMIES    = MAX_N_AMMUNITION;
